from our_datasets import move_to_cuda
from utils.lib import *
from utils.dist import is_main_process, get_world_size, reduce_dict, get_local_rank
...
class Agent_Base:
    def __init__(self, args, model):
        ...
        if hasattr(model, "tokzr") and self.model.tokzr is not None:
            ...
            self.tokzr = transformers.AutoTokenizer.from_pretrained(args.tokenizer)
        (
            self.cls_token_id,
            self.sep_token_id,
            self.pad_token_id,
    ...
    def build_optimizer(self):
        ...
        decay_other_param = [
            ...
            if "swin." not in n and "regressor_aqa" not in n
        ]
        ...
        decay_score_param = [(n, p) for n, p in decay_param_tp if "regressor_aqa" in n]
        no_decay_score_param = [
            (n, p) for n, p in no_decay_param_tp if "regressor_aqa" in n
        ]
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": [p for n, p in decay_other_param], "weight_decay": weight_decay},
            {  # 添加 AQA 模块的参数组
                "params": [p for n, p in decay_score_param],
                "weight_decay": weight_decay,
                "lr": 0.0005,  # 使用 AQA 模块的学习率
            },
            {
            ...
            {"params": [p for n, p in no_decay_other_param], "weight_decay": 0.0},
            
            {  # 添加 AQA 模块的参数组 (no decay)
                "params": [p for n, p in no_decay_score_param],
                "weight_decay": 0.0,
                "lr": 5e-4,  # 使用 AQA 模块的学习率
            },
        ]
    ...
    def save_training_meta(self):
        if is_main_process():
            ...
            print(self.args)
            json.dump(
                self.args, open(f"{self.args.path_output}/args.json", "w"), indent=2
            )
            self.save_model(0)
    ...
    def save_model(self, ep):
        if is_main_process():
            ...
            if self.log is not None:
                json.dump(self.log, open(f"{output_dir}/log.json", "w"), indent=2)
        # DIST.barrier()
        # synchronize()
    ...
    def log_memory(self, ep=-1, step=-1):
        ...
        lr_aqa = f"{self.optzr.param_groups[2]['lr']:.2e}"
        return (
            f"{step_str} lr_swin: {lr_swin}, "
            + f"lr_bert: {lr_bert},lr_aqa: {lr_aqa}, max memory: {memory}"
        )
    ...
    def forward_step(self, batch):
        if self.args.deepspeed:
            if isinstance(batch, dict):
                ...
                raise TypeError(f"batch is either dict or tuple, {type(batch)}")
        else:
